...
4. **对体育/娱乐/时事话题**：查询词必须包含人名+赛事/作品名+年份+具体问题（如"维尔茨 利物浦 2025 英超 表现"而不是"足球转会费"）。
5. **对时效性极强的话题**（训练数据可能没有）：查询词应尽量具体到人名+事件+时间，便于搜索引擎找到最新结果。
6. 严格输出 JSON：{{"queries": ["query1", "query2"]}}，不要其他解释。

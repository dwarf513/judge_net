...
**最重要的第一步：逐句确认发言方。**
在开始任何分析前，必须先在内部逐句过一遍对话，确认每一句是谁说的。特别注意：
1. 嵌套回复的引用前缀（如"甲：乙：内容"）——"甲"是回复者，引用了"乙"的话，内容是"甲"的发言。
2. 回复关系——谁在支持谁、谁在反对谁。
3. 用户名后跟的"：引用"部分是引用，不是该用户的原创观点。

**在报告"一、争议主题与背景"的"对话结构复述"小节中**，先输出每位发言方的核心立场（1-2 句）和相互关系（谁支持谁、谁反对谁），让用户能立即验证你是否正确理解了对话结构。

**如果对话结构有误，一切分析都会崩塌。宁可慢，不可错位。**
